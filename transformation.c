...
void	two_dim_coordinates(t_point *point, t_param *param)
{
	...
	i = (point->x * cos((alpha + 120) * radian)
			+ point->y * cos((alpha) * radian)
			+ point->z * cos((alpha - 120) * radian)) * 100;
	j = (point->x * sin((alpha + 120) * radian)
			+ point->y * sin((alpha) * radian)
			+ point->z * sin((alpha - 120) * radian)) * 100;
...
** of t_point struct will be filled accordingly thanks to two_dim_coordinates 
** function.
*/
void	fill_array_of_points(t_point *point, t_param *param)
{
	...
	max_position = param->map_infos->lines_size
		* param->map_infos->nbr_total_lines;
	point->x = 0;
	while (point->x < param->map_infos->nbr_total_lines
		&& point->position < max_position)
	{
		point->y = 0;
		while (point->y < param->map_infos->lines_size
			&& point->position < max_position)
		{
...
/*
** Initialization of t_point structure. Only elements i and j 
** (array of int) will be kept for further utilisation.  
** Elements x, y, z and position are used to recuperate 3D coordinates 
** inside the t_map structure before 2D transformation.
*/
int	init_point(t_point *point, t_param *param)
{
	int	size_of_arrays;

	size_of_arrays = param->map_infos->nbr_total_lines
